...
4. 오른쪽 위 **✍️ 편지 쓰기** 버튼을 누르면 — 로그인 상태라면 제목/내용, 열람 허용 반경(30m~1km), 그리고(연인이 등록되어 있다면) **받는 사람**을 정해 편지를 남길 수 있어요. 연인이 등록되어 있으면 받는 사람은 기본으로 **"❤️ 연인에게만"**이 선택돼요(필요하면 전체 공개로 바꿀 수 있어요). 로그인하지 않았다면 로그인 화면으로 안내돼요.
...
10. 로그인 상태에서는 오른쪽 목록에 **[내가 남긴 편지] / [나에게 남겨진 편지]** 탭이 나타나요. 전자는 내가 작성한 편지만, 후자는 **연인이 나를 받는 사람으로 지정해 남긴 편지만** 보여줍니다(다른 사람이 남긴 전체 공개 편지는 이 탭에 나타나지 않아요). 각 탭 안에서 **가까운 순 / 최신 순** 정렬도 고를 수 있어요.
11. 목록 위에는 **전체 / ❤️ 연인 / 공개** 필터가 있어요. "❤️ 연인"을 고르면 연인에게만 공개된 편지만, "공개"를 고르면 전체 공개 편지만 보여줍니다. 이 필터는 오른쪽 목록뿐 아니라 **지도에 표시되는 마커에도 동일하게 적용**돼요.
12. 내가 쓴 편지가 열람되면 **읽은 일시가 기록**돼요. 내가 쓴 편지를 다시 열어보면(목록에서는 "✔️ 읽음"/"아직 안 읽음"으로, 열람 화면에서는 누가 몇 시에 읽었는지) 확인할 수 있어요. 처음 읽힌 시각만 기록되고, 이후 다시 읽어도 시각은 바뀌지 않아요.
13. **전체 공개 편지는 현재 위치에서 1km 이내에 있는 것만** 지도와 목록에 나타나요(너무 멀리 있는 공개 편지로 지도가 복잡해지지 않도록). 연인에게 온 편지나 내가 쓴 편지는 거리와 상관없이 항상 보여요.
14. 편지 목록 상단에는 **총 편지 개수**가 뱃지로 표시돼요(100통이 넘으면 "99+"로 표기). 목록은 한 번에 10통까지만 보여주고, 더 있으면 하단의 **더 보기** 버튼을 눌러 10통씩 추가로 볼 수 있어요.
15. 모바일에서는 편지 목록이 없을 때도 손잡이를 탭하거나 위로 드래그하면 패널을 넓게 펼칠 수 있어요.
...
- **대상 지정 편지**: 편지를 쓸 때 받는 사람을 "전체 공개"(기본) 또는 "등록된 연인"으로 고를 수 있습니다. 대상이 지정된 편지는 **위치 조건과 별개로, 로그인한 사용자가 그 대상과 일치할 때만** 서버가 본문을 내려줍니다 — 같은 자리에 있어도 다른 사람에게는 잠금 사유만("정해진 한 사람만 열 수 있는 편지") 전달되고 본문은 응답에 아예 포함되지 않습니다. 연인에게 쓴 편지는 봉인하는 시점에 작성자의 연애 시작일 기준 며칠째인지를 함께 계산해 저장하고, 목록과 열람 화면에 하트+숫자 뱃지(❤️ 7 처럼)로 표시합니다. 나중에 연애 시작일을 수정하더라도 그 편지가 쓰인 날의 기록은 바뀌지 않습니다.
- **관리자 계정**: 아이디가 정확히 `admin`인 계정은 자동으로 관리자 권한을 가집니다(가입 시 자동 부여, 이미 존재하던 `admin` 계정도 서버가 켜질 때 자동으로 승격됩니다). 관리자로 로그인하면 **위치나 대상 지정과 상관없이 모든 편지를 열람**할 수 있어요 — 목록/열람 API가 항상 `unlocked: true`로 응답합니다. 편지 삭제 권한 등 다른 제약은 그대로 적용됩니다.
- **읽음 확인**: 편지 작성자가 아닌 다른 사용자가 로그인한 상태로 편지 본문을 성공적으로 열람하면(`GET /letters/{id}`가 `unlocked: true`를 응답할 때), 서버가 그 편지의 `read_at`/`read_by_username`을 최초 1회만 기록합니다(이미 기록이 있으면 갱신하지 않음). 목록/열람 API 응답에 함께 내려오며, 작성자 화면에서만 표시됩니다.
- **테스트 계정**: 개발 중 쌓인 임시 계정/편지는 한 번 정리했고, 앞으로 테스트는 **`test001` ~ `test005`** 계정(비밀번호 모두 `asdf1234`)으로만 진행합니다. `admin` 계정은 그대로 유지됩니다. 이 정리는 `SchemaInitializer`의 1회성 마이그레이션(`schema_migrations` 테이블로 재실행 방지)으로 수행됐으며, 이후 배포에서는 다시 실행되지 않습니다.
...
- `src/components/LetterPanel.jsx` — 편지 목록 (전체/❤️ 연인/공개 필터, 모바일에서는 손잡이를 탭하거나 위아래로 드래그해 목록 패널을 넓게/좁게 볼 수 있어요)
