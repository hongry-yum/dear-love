...
- **대상 지정 편지**: 편지를 쓸 때 받는 사람을 "전체 공개"(기본) 또는 "등록된 연인"으로 고를 수 있습니다. 대상이 지정된 편지는 **위치 조건과 별개로, 로그인한 사용자가 그 대상과 일치할 때만** 서버가 본문을 내려줍니다 — 같은 자리에 있어도 다른 사람에게는 잠금 사유만("정해진 한 사람만 열 수 있는 편지") 전달되고 본문은 응답에 아예 포함되지 않습니다.
